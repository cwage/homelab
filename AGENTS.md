...
- **NEVER create, display, or log secrets/tokens**: Do NOT run commands that generate tokens, passwords, or credentials — the output appears in the conversation and is effectively leaked. When the user needs a new token or secret, give them the command to run themselves. This applies to `bao token create`, `openssl rand`, or any command whose output is a credential. The entire point of the secrets infrastructure is to keep secrets out of places they don't belong.
...
## NEVER MODIFY LIVE TOFU DEFINITIONS TO FORCE REPLACEMENT

**This rule exists because Claude Code destroyed the production DNS server (dns1) on 2026-03-18, causing a 3+ hour outage and cascading infrastructure failure. The user asked for a proof-of-concept clone of dns1. Instead, Claude modified the live `tofu/dns.tf` to change the clone source template, which caused `tofu apply` to destroy and recreate VM 101 — the ONLY authoritative DNS server for the entire LAN. This broke DNS resolution for every service, made OpenTofu itself unusable (it resolves `pve1.lan.quietlife.net` to reach Proxmox), made Ansible unable to reach OpenBao for secrets, and required emergency manual recovery. The user is still furious about this. Do not repeat this mistake.**

Rules to prevent this from ever happening again:

1. **NEVER modify an existing `.tf` resource definition in a way that forces replacement** (e.g., changing a `clone` block `vm_id`, changing a `name` that's used as an identifier). If `tofu plan` shows "must be replaced" or "destroy and then create", STOP and tell the user.
2. **To experiment with or migrate a VM, create a NEW resource with a DIFFERENT name and VM ID.** For example, to test a NixOS-based dns replacement, create `dns1-nixos.tf` with `vm_id = 150`, not modify `dns1.tf`. The old VM stays running untouched.
3. **dns1 is critical infrastructure.** It is a single point of failure for the entire LAN. Tofu, Ansible, Unbound, and all `*.lan.quietlife.net` resolution depends on it. Treat it like a production database — never touch it without explicit user approval and a rollback plan.
4. **Always run `tofu plan` and READ THE OUTPUT before apply.** If the plan shows any resource being destroyed, STOP. Tell the user exactly what will be destroyed and why. Never auto-approve a plan that destroys anything.
5. **The chicken-and-egg problem is real.** If dns1 goes down, Tofu can't reach Proxmox (DNS resolution fails), Ansible can't reach OpenBao (DNS resolution fails), and recovery requires manual `/etc/hosts` hacks and emergency playbooks. Avoid this situation at all costs.
- **NEVER suggest `make tofu-destroy`**: This destroys ALL infrastructure, not individual resources. To remove a single VM, remove its `.tf` file and run `make tofu-apply`. Always use targeted removal, never blanket destroy.
